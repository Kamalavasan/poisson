...
void process_SLR (hls::stream <t_pkt> &in, hls::stream <t_pkt> &out,
    ...
    process_grid( streamArray[0], streamArray[1], size_x, size_y, tile_x, data_g);
    process_grid( streamArray[1], streamArray[2], size_x, size_y, tile_x, data_g);
    process_grid( streamArray[2], streamArray[3], size_x, size_y, tile_x, data_g);
    process_grid( streamArray[3], streamArray[4], size_x, size_y, tile_x, data_g);

    process_grid( streamArray[4], streamArray[5], size_x, size_y, tile_x, data_g);
    process_grid( streamArray[5], streamArray[6], size_x, size_y, tile_x, data_g);
    process_grid( streamArray[6], streamArray[7], size_x, size_y, tile_x, data_g);
    process_grid( streamArray[7], streamArray[8], size_x, size_y, tile_x, data_g);

    process_grid( streamArray[8], streamArray[9], size_x, size_y, tile_x, data_g);
...
void stencil_SLR2(
	...
	for(int i =  0; i < 2*count ; i++){
		...
		for(j = 0; j < tile_count; j++){
			...
			process_SLR( in, out, xdim0, offset, tile_x, sizey);
		}
	}
