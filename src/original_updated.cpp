...
void ops_init_backend() {
	cl_int ret;
			...
			//fpga_bin_size = load_file_to_memory("/home/faraz/workspace/Poisson/System/ops_poisson_kernel_populate.xclbin", &fpga_bin);
			fpga_bin_size = load_file_to_memory("../ops_poisson_kernels.xclbin", &fpga_bin);
			printf("Populate File size: %d \n", fpga_bin_size);
...
void ops_decl_const_char( int dim, char const * type, int typeSize, char * dat, char const * name ) {
    ...
    d_bank0_ext.param = 0;*/
			...
			OPS_opencl_core.constant[0] = clCreateBuffer(OPS_opencl_core.context, CL_MEM_READ_ONLY, dim*typeSize, nullptr, &ret);
			//OPS_opencl_core.constant[0] = clCreateBuffer(OPS_opencl_core.context, CL_MEM_READ_ONLY, dim*typeSize, &d_bank0_ext, &ret);
			clSafeCall( ret );
    	...
    	d_bank1_ext.flags = 1 | XCL_MEM_TOPOLOGY;
    	    ...
    	    d_bank1_ext.param = 0;*/
				...
				OPS_opencl_core.constant[1] = clCreateBuffer(OPS_opencl_core.context, CL_MEM_READ_ONLY, dim*typeSize, nullptr, &ret);
				clSafeCall( ret );
...
void buildOpenCLKernels_poisson_kernel_update(int xdim0, int xdim1) {
	...
	if(!isbuilt_poisson_kernel_update) {
		...
		/*char *fpga_bin;
            ...
            fpga_bin_size = load_file_to_memory("../ops_poisson_kernels.xclbin", &fpga_bin);
            printf("Update File: %d \n", fpga_bin_size);
            OPS_opencl_core.program = clCreateProgramWithBinary(OPS_opencl_core.context, 1, (const cl_device_id*) &OPS_opencl_core.device_id, &fpga_bin_size, (const unsigned char**) &fpga_bin, NULL, &ret);

         clSafeCall( ret );*/
		printf("Building update \n");
	...
	/*	char buildOpts[] = " ";
		...
		printf("compiling poisson_kernel_update\n");*/
...
void ops_par_loop_poisson_kernel_update(char const *name, ops_block block, int dim, int* range,
	...
	if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
		...
		cl_int ret;
		cl_mem buff0 = clCreateBuffer(OPS_opencl_core.context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(cl_mem),  arg0.data_d, &ret);
		clSafeCall( ret );

		cl_mem buff1 = clCreateBuffer(OPS_opencl_core.context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(cl_mem),  arg1.data_d, &ret);
		clSafeCall( ret );
		...
		clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 0, sizeof(cl_mem), buff0 ));
		clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 1, sizeof(cl_mem), buff1 ));
...
void ops_par_loop_poisson_kernel_populate(char const *name, ops_block block, int dim, int* range,
	...
	ops_printf("Populate dims: %d, %d, %d\n", xdim3, xdim4, xdim5);
	//build opencl kernel if not already built
	...
	if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
		...
		//cl_mem buff3 = clCreateBuffer(OPS_opencl_core.context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(cl_mem), &arg3.data_d, &ret);
		//cl_mem buff3 = clCreateBuffer(OPS_opencl_core.context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(cl_mem), &arg3.data_d, &ret);
		...
		cl_mem buff4 = clCreateBuffer(OPS_opencl_core.context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(cl_mem),  arg4.data_d, &ret);
		clSafeCall( ret );

		cl_mem buff5 = clCreateBuffer(OPS_opencl_core.context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, sizeof(cl_mem),  arg5.data_d, &ret);
		clSafeCall( ret );


		//clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 2, sizeof(cl_mem), (void*) &buff3 ));
		//clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 3, sizeof(cl_mem), (void*) &buff4 ));
		//clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 4, sizeof(cl_mem), (void*) &buff5 ));
		clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 2, sizeof(cl_mem), &buff3 ));
		clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 3, sizeof(cl_mem), &buff4 ));
		clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[0], 4, sizeof(cl_mem), &buff5 ));
	...
	//	clSafeCall( clFinish(OPS_opencl_core.command_queue) );
		...
		//clSafeCall( clFinish(OPS_opencl_core.command_queue) );
		//clSafeCall( clEnqueueReadBuffer(OPS_opencl_core.command_queue, buff3, CL_TRUE, 0, sizeof(cl_mem), &arg3.data_d , 0, NULL, NULL) );
		//clSafeCall( clFinish(OPS_opencl_core.command_queue) );
		//ops_download_dat(arg3.dat);

		/*int bytes = arg3.dat->elem_size;
    for (int i = 0; i < arg3.dat->block->dims; i++)
    	bytes = bytes * arg3.dat->size[i];

    cl_mem buff = clCreateBuffer(OPS_opencl_core.context, CL_MEM_USE_HOST_PTR | CL_MEM_READ_WRITE, bytes, &arg3.data_d, &ret);
    clSafeCall( ret );*/
		...
		//clSafeCall(clEnqueueReadBuffer(OPS_opencl_core.command_queue, buff, CL_TRUE, 0, bytes, &arg3.data_d, 0, NULL, NULL));
		// clSafeCall( clFlush(OPS_opencl_core.command_queue) );


		//ops_printf("BOSS4: arg0:%d, arg1:%d, base3:%d, base4:%d, base5:%d, arg_idx0:%d, arg_idx1:%d, size0:%d, size1:%d, dx:%f, dy:%f, arg3:%f, arg4:%f, arg5:%f \n", atoi(arg0.data), atoi(arg1.data), base3, base4, base5, arg_idx[0], arg_idx[1], x_size, y_size, dx, dy, atof(arg3.data_d), atof(arg4.data_d), atof(arg5.data_d));
	}
	if (OPS_diags>1) {
		clSafeCall( clFinish(OPS_opencl_core.command_queue) );
	}
