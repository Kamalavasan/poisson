...
__kernel void ops_poisson_kernel_populate(
		...
		int base_index3, base_index4, base_index5, end_index;
		...
		for(int i  = 0; i < size1; i++){
			...
			for(int j = 0; j < end_index; j++){
				...
				for(int k = 0; k < PORT_WIDTH; k++){
					...
					float f3 = myfun(sin(M_PI*x),cos(2.0*M_PI*y))-1.0;
					float f4 = -5.0*M_PI*M_PI*sin(M_PI*x)*cos(2.0*M_PI*y);
					float f5 = sin(M_PI*x)*cos(2.0*M_PI*y);
