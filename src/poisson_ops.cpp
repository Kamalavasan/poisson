...
int main(int argc,  char **argv)
{
	...
	//posix_memalign((void**)&blocks,4096,ngrid_x*ngrid_y*sizeof(ops_block*));
	char buf[50];
	...
	//posix_memalign((void**)&coordx,4096,ngrid_x*ngrid_y*sizeof(ops_dat*));

	ops_dat *coordy = (ops_dat *)malloc(ngrid_x*ngrid_y*sizeof(ops_dat*));
	//posix_memalign((void**)&coordy,4096,ngrid_x*ngrid_y*sizeof(ops_dat*));

	ops_dat *u = (ops_dat *)malloc(ngrid_x*ngrid_y*sizeof(ops_dat*));
	//posix_memalign((void**)&u,4096,ngrid_x*ngrid_y*sizeof(ops_dat*));

	ops_dat *u2 = (ops_dat *)malloc(ngrid_x*ngrid_y*sizeof(ops_dat*));
	//posix_memalign((void**)&u2,4096,ngrid_x*ngrid_y*sizeof(ops_dat*));

	ops_dat *f = (ops_dat *)malloc(ngrid_x*ngrid_y*sizeof(ops_dat*));
	//posix_memalign((void**)&f,4096,ngrid_x*ngrid_y*sizeof(ops_dat*));

	ops_dat *ref = (ops_dat *)malloc(ngrid_x*ngrid_y*sizeof(ops_dat*));
	//posix_memalign((void**)&ref,4096,ngrid_x*ngrid_y*sizeof(ops_dat*));

	int *sizes = (int*)malloc(2*ngrid_x*ngrid_y*sizeof(int));
	//posix_memalign((void**)&sizes,4096,2*ngrid_x*ngrid_y*sizeof(int));

	int *disps = (int*)malloc(2*ngrid_x*ngrid_y*sizeof(int));
	//posix_memalign((void**)&disps,4096,2*ngrid_x*ngrid_y*sizeof(int));
	...
	//posix_memalign((void**)&halos,4096,2*(ngrid_x*(ngrid_y-1)+(ngrid_x-1)*ngrid_y)*sizeof(ops_halo));
	int off = 0;
	for (int j = 0; j < ngrid_y; j++) {
		for (int i = 0; i < ngrid_x; i++) {
			if (i > 0) {
	...
	for (int j = 0; j < ngrid_y; j++) {
		for (int i = 0; i < ngrid_x; i++) {
			...
			ops_par_loop_poisson_kernel_initial("poisson_kernel_initial", blocks[i+ngrid_x*j], 2, iter_range,
					ops_arg_dat(u[i+ngrid_x*j], 1, S2D_00, "double", OPS_WRITE));
	...
	ops_printf("\nTotal Wall time %lf\n",et1-et0);
	double err_diff=fabs((100.0*(err/20.7270070946193))-100.0);
