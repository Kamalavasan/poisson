...
// host stub function
void ops_par_loop_poisson_kernel_stencil(char const *name, ops_block block, int dim, int niters, int* range,
...
//	size_t localWorkSize[3] =  {OPS_block_size_x,OPS_block_size_y,OPS_block_size_z};
	...
	if (globalWorkSize[0]>0 && globalWorkSize[1]>0 && globalWorkSize[2]>0) {
		...
		clSafeCall( clSetKernelArg(OPS_opencl_core.kernel[3], 8, sizeof(cl_int), (void*) &niters ));
