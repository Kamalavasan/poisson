static void derives_calc_ytep_k4( hls::stream<uint256_dt> &rd_buffer, hls::stream<uint256_dt> &yy_read, hls::stream<uint256_dt> &yy_final_read, hls::stream<uint256_dt> &wr_buffer, hls::stream<uint256_dt> &yy_write, hls::stream<uint256_dt> &yy_final_write, struct data_G data_g){
	...
	for(unsigned int itr = 0; itr < gridsize; itr++) {
		...
		vzz *= invdz;
  		...
  		// YY final 
  		yy_final_arr[2] += mem_wr_k_dt[2] * 0.16666666;
  		yy_final_arr[5] += mem_wr_k_dt[5] * 0.16666666;

  		yy_final_arr[3] += mem_wr_k_dt[3] * 0.16666666;
  		yy_final_arr[6] += mem_wr_k_dt[6] * 0.16666666;

  		yy_final_arr[4] += mem_wr_k_dt[4] * 0.16666666;
  		yy_final_arr[7] += mem_wr_k_dt[7] * 0.16666666;

  		yy_final_arr[0] += mem_wr_k_dt[0] * 0.16666666;
  		yy_final_arr[1] += mem_wr_k_dt[1] * 0.16666666;
