...
//
//
//static void stream_convert_512_256(hls::stream<uint512_dt> &in, hls::stream<uint256_dt> &out, unsigned int tot_itr){
//
//	for (int itr = 0; itr < tot_itr; itr++){
//		#pragma HLS PIPELINE II=1
//		#pragma HLS loop_tripcount min=min_grid max=max_grid avg=avg_grid
...
//}
//
//static void stream_convert_256_512(hls::stream <uint256_dt> &in, hls::stream <uint512_dt> &out, unsigned int tot_itr){
//
//	for (int itr = 0; itr < tot_itr; itr++){
//		#pragma HLS PIPELINE II=1
//		#pragma HLS loop_tripcount min=min_grid max=max_grid avg=avg_grid
...
static void process_ReadWrite (uint512_dt*  arg0_1, uint512_dt*  arg1_1,
...
//
	...
	read_tile(arg0_1, rd_bufferArr[0], data_g, 0);
	fifo512_2axis(rd_bufferArr[0], outl, outu, totol_iter_R);
	axis2_fifo512(inl, inu, wr_bufferArr[0], totol_iter_W);
	write_tile(arg1_1, wr_bufferArr[0], data_g, 0);
...
static void process_ReadWrite_dataflow ( uint512_dt*  arg0_1, uint512_dt*  arg1_1,
				   hls::stream <t_pkt> &inl, hls::stream <t_pkt> &inu,
				   hls::stream <t_pkt> &outl, hls::stream <t_pkt> &outu,
...
//		printf("Itr:%d\n", itr);
		...
		process_ReadWrite(arg0_1,  arg1_1,  inl, inu, outl, outu, xdim0, offset_x, tile_x, offset_y, tile_y, size_x, size_y, size_z);
		j++;
	}
}
...
void stencil_Read_Write_SLR1(
	...
	#pragma HLS INTERFACE depth=4096 m_axi port = arg0_1 offset = slave bundle = gmem1 max_read_burst_length=64 max_write_burst_length=64 num_read_outstanding=16 num_write_outstanding=16 latency=30
	#pragma HLS INTERFACE depth=4096 m_axi port = arg1_1 offset = slave bundle = gmem1 max_read_burst_length=64 max_write_burst_length=64 num_read_outstanding=16 num_write_outstanding=16 latency=30
	...
	for(int i =  0; i < count; i++){
		process_ReadWrite_dataflow(arg0_1, arg1_1, inl, inu, outl, outu, xdim0, tile_memx, tilex_count, tile_memy, tiley_count, sizex, sizey, sizez);
		process_ReadWrite_dataflow(arg1_1, arg0_1, inl, inu, outl, outu, xdim0, tile_memx, tilex_count, tile_memy, tiley_count, sizex, sizey, sizez);
	}
}
}
