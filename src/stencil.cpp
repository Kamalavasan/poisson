...
static void axis2_fifo256_5(hls::stream <t_pkt_1024> &in0, hls::stream <t_pkt> &in1,
	...
	for (unsigned int itr = 0; itr < total_itr; itr++){
		...
		uint256_dt u_data = register_it <uint256_dt> (tmp1.data);

		out0 << l_data.range(255,0);
		out1 << l_data.range(511,256);
		out2 << l_data.range(767,512);
		out3 << l_data.range(1023,768);

		out4 << u_data.range(255,0);
...
static void fifo256_5_2axis(hls::stream <uint256_dt> &in0, hls::stream <uint256_dt> &in1,
		hls::stream <uint256_dt> &in2, hls::stream <uint256_dt> &in3,
		hls::stream <uint256_dt> &in4,
		hls::stream<t_pkt_1024> &out0, hls::stream<t_pkt> &out1, unsigned int total_itr){
	...
	for (unsigned int itr = 0; itr < total_itr; itr++){
		...
		t_pkt tmp1;
		uint1024_dt l_data;
		l_data.range(255,0)    = in0.read();
		l_data.range(511,256)  = in1.read();
		l_data.range(767,512)  = in2.read();
		l_data.range(1023,768) = in3.read();


		tmp0.data = register_it <uint1024_dt>(l_data);
		tmp1.data = register_it <uint256_dt>(in4.read());
		out0.write(tmp0);
		out1.write(tmp1);
	}
}
...
static void process_tile( hls::stream<uint256_dt> &rd_buffer0, hls::stream<uint256_dt> &rd_buffer1,
		hls::stream<uint256_dt> &rd_buffer2, hls::stream<uint256_dt> &rd_buffer3,  hls::stream<uint256_dt> &rd_buffer4,
		hls::stream<uint256_dt> &wr_buffer0, hls::stream<uint256_dt> &wr_buffer1,
		hls::stream<uint256_dt> &wr_buffer2, hls::stream<uint256_dt> &wr_buffer3,  hls::stream<uint256_dt> &wr_buffer4,
		short offset_v,
	...
	uint640_dt window_1_l[MAX_DEPTH_P];
	uint640_dt window_2_l[MAX_DEPTH_L*2];
	uint640_dt window_3_l[MAX_DEPTH_L*2];
	uint640_dt window_4_l[MAX_DEPTH_P];

	uint640_dt window_1_u[MAX_DEPTH_P];
	uint640_dt window_2_u[MAX_DEPTH_L*2];
	uint640_dt window_3_u[MAX_DEPTH_L*2];
	uint640_dt window_4_u[MAX_DEPTH_P];
	...
	uint640_dt s_1_1_2_l, s_1_2_1_l, s_1_1_1_l, s_1_1_1_l_b, s_1_1_1_l_f, s_1_0_1_l, s_1_1_0_l;
	uint640_dt s_1_1_2_u, s_1_2_1_u, s_1_1_1_u, s_1_1_1_u_b, s_1_1_1_u_f, s_1_0_1_u, s_1_1_0_u;
	uint640_dt update_j_l, update_j_u;
	...
	for(unsigned int itr = 0; itr < gridsize; itr++) {
		...
		if(cond_tmp1){
			uint640_dt tmp0, tmp1;
			uint256_dt rd0 = rd_buffer0.read();
			uint256_dt rd1 = rd_buffer1.read();
			uint256_dt rd2 = rd_buffer2.read();
			uint256_dt rd3 = rd_buffer3.read();
			uint256_dt rd4 = rd_buffer4.read();

			tmp0.range(255,0)   = rd0;
			tmp0.range(511,256) = rd1;
			tmp0.range(639,512) = rd2.range(127,0);

			tmp1.range(127,0)   = rd2.range(255,128);
			tmp1.range(383,128) = rd3;
			tmp1.range(639,384) = rd4;
		...
		process_l: for(short q = 0; q < PORT_WIDTH; q++){
			#pragma HLS loop_tripcount min=port_width max=port_width avg=port_width
			short index = (k << (SHIFT_BITS))*5 + q + offset_x + offset_v;
...
//			#pragma HLS RESOURCE variable=f3 core=FAddSub_nodsp
		...
		if(cond_wr ) {
			...
			uint256_dt tmp0 = update_j_l.range(255,0);
			uint256_dt tmp1 = update_j_l.range(511,256);
			uint256_dt tmp2;
			tmp2.range(127,0) = update_j_l(639,512);
			tmp2.range(255,128) = update_j_u(127,0);
			uint256_dt tmp3 = update_j_u.range(383,128);
			uint256_dt tmp4 = update_j_u.range(639,384);
